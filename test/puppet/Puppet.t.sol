...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        Exploit exploit = new Exploit(token, lendingPool, uniswapV1Exchange);
        token.transfer(address(exploit), PLAYER_INITIAL_TOKEN_BALANCE);
        exploit.attack{value: 20 ether}(recovery);

        console2.log("Player Final ETH balance", player.balance);
    }
