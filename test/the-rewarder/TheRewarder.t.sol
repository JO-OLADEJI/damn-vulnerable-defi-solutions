...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        uint256 playerRewardIndex = 188;
        bytes32[] memory dvtClaimLeaves = _loadRewards(
            "/test/the-rewarder/dvt-distribution.json"
        );
        bytes32[] memory wethClaimLeaves = _loadRewards(
            "/test/the-rewarder/weth-distribution.json"
        );

        Reward memory playerDvtRewards = _getRewardAtIndex(
            playerRewardIndex,
            "/test/the-rewarder/dvt-distribution.json"
        );
        Reward memory playerWethRewards = _getRewardAtIndex(
            playerRewardIndex,
            "/test/the-rewarder/weth-distribution.json"
        );

        assert(playerDvtRewards.beneficiary == player);
        assert(playerWethRewards.beneficiary == player);

        IERC20[] memory tokensToClaim = new IERC20[](2);
        tokensToClaim[0] = IERC20(address(dvt));
        tokensToClaim[1] = IERC20(address(weth));

        uint256 noOfDvtClaims = dvt.balanceOf(address(distributor)) /
            playerDvtRewards.amount;
        uint256 noOfWethClaims = weth.balanceOf(address(distributor)) /
            playerWethRewards.amount;

        bytes32[] memory playerDvtproof = merkle.getProof(
            dvtClaimLeaves,
            playerRewardIndex
        );
        bytes32[] memory playerWethproof = merkle.getProof(
            wethClaimLeaves,
            playerRewardIndex
        );

        Claim[] memory claims = new Claim[](noOfDvtClaims + noOfWethClaims);
        for (uint256 i = 0; i < noOfDvtClaims + noOfWethClaims; ) {
            if (i < noOfDvtClaims) {
                claims[i] = Claim({
                    batchNumber: 0,
                    amount: playerDvtRewards.amount,
                    tokenIndex: 0,
                ...
                claims[i] = Claim({
                    batchNumber: 0,
                    amount: playerWethRewards.amount,
                    tokenIndex: 1,
